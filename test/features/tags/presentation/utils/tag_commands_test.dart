...
void main() {
  ...
  group('UpdateTagCommand', () {
    ...
    test(
      'should rename tag, move icon, and update task titles on execute and undo',
      () async {
        ...
        when(
          () => mockIconRepo.getAllIconDatas(),
        ).thenAnswer((_) async => {'work': Icons.work});
        when(
          () => mockIconRepo.deleteIconDataForTag('work'),
        ).thenAnswer((_) async {});
        when(
          () => mockIconRepo.setIconDataForTag('job', Icons.work),
        ).thenAnswer((_) async {});
        when(
          () => mockIconRepo.setIconDataForTag('work', Icons.work),
        ).thenAnswer((_) async {});
        when(
          () => mockIconRepo.deleteIconDataForTag('job'),
        ).thenAnswer((_) async {});

        final task1 = Task(
          id: 'task1',
          title: 'Finish #work stuff',
          createdAt: DateTime.now(),
          tagIds: const ['t1'],
        );
        final task2 = Task(
          id: 'task2',
          title: 'Other task #workplace',
          createdAt: DateTime.now(),
          tagIds: const ['t1', 't2'],
        );
        when(
          () => mockTaskRepo.getAll(prioritizeDeadlines: false),
        ).thenAnswer((_) async => [task1, task2]);
        when(
          () => mockTaskRepo.getById('task1'),
        ).thenAnswer((_) async => task1.copyWith(title: 'Finish #job stuff'));
        ...
        final capturedTasks = verify(
          () => mockTaskRepo.update(captureAny()),
        ).captured.cast<Task>();
  ...
  group('DeleteTagCommand', () {
    ...
    test(
      'should delete tag, delete icon, strip hashtag and update tagIds on execute and undo (including picker-only tags)',
      () async {
        ...
        when(() => mockTaskRepo.getById('task1')).thenAnswer(
          (_) async => task1.copyWith(title: 'Finish stuff', tagIds: []),
        );
        when(
          () => mockTaskRepo.getById('task2'),
        ).thenAnswer((_) async => task2.copyWith(tagIds: ['t2']));
