...
void main() {
  group('filter', () {
    ...
    test(
      'applyToTask project matching handles inclusion and exclusion correctly',
      () {
        ...
        expect(
          excFilter.applyToTask(taskNoProject, []),
          isTrue,
        ); // No project task doesn't match excluded project ID p3
        ...
        expect(
          comboFilter.applyToTask(taskInP3, []),
          isFalse,
        ); // p3 is excluded, so false
      },
    );

    test(
      'applyToTask label matching handles inclusion, exclusion, and inheritance correctly',
      () {
        ...
        expect(
          incFilter.applyToTask(taskWithNoLabels, ['l1']),
          isTrue,
        ); // Inherited L1
        ...
        expect(
          excFilter.applyToTask(taskWithNoLabels, ['l2']),
          isFalse,
        ); // Excluded label is inherited
        ...
        expect(
          comboFilter.applyToTask(taskWithL2, []),
          isFalse,
        ); // L2 is excluded, so false
      },
    );
    ...
    test(
      ...
      () {
        ...
        final matchProject = createProject(
          id: 'p1',
          isUrgent: true,
          labelIds: ['l1'],
        );
        final wrongPriority = createProject(
          id: 'p2',
          isUrgent: false,
          labelIds: ['l1'],
        );
        final wrongLabel = createProject(
          id: 'p3',
          isUrgent: true,
          labelIds: [],
        );
        final excludedLabel = createProject(
          id: 'p4',
          isUrgent: true,
          labelIds: ['l1', 'l2'],
        );
